...
def draw_default_bar(stock_df, title, step=2):
    # 按时间升序来展示
    stock_df = stock_df[::-1]
    x = stock_df['amount'].index
    y = stock_df['amount'].values
    x_label = stock_df['trade_date'].values
...
def draw_field_plot(stock_df, field, title, step=2):
    # 按时间升序来展示
    stock_df = stock_df[::-1]
    x = stock_df[field].index
    y = stock_df[field].values
    x_label = stock_df['trade_date'].values
...
# 绘制折线图和条形图
def draw_compare_plot_bar(x1, y1, y2, x_label, y1_label, y2_label, title, step=10):
    log.info('---- 绘制折线和条形图 ----')
    plt.figure(figsize=(20, 8), dpi=80)
    fig, axs = plt.subplots(2, 1)
    # 绘制数据
...
def draw_default_compare_plot(stock_df, index_df, title, step=10):
    ...
    stock_df = stock_df[::-1]
    index_df = index_df[::-1]
    x1 = stock_df['close'].index
    y1 = stock_df['close'].values
    y2 = index_df['close'].values
    x_label = stock_df['trade_date'].values
...
def draw_default_compare_plot_bar(stock_df, title, step=10):
    ...
    x1 = stock_df['close'].index
    y1 = stock_df['close'].values
    y2 = stock_df['amount'].values
    x_label = stock_df['trade_date'].values

    draw_compare_plot_bar(x1, y1, y2, x_label, 'stock', 'amount', title, step)


def draw_field_compare_plot_bar(stock_df, field, title, step=10):
    # 按时间升序来展示
    stock_df = stock_df[::-1]
    x1 = stock_df['close'].index
    y1 = stock_df['close'].values
    y2 = stock_df[field].values
    x_label = stock_df['trade_date'].values

    draw_compare_plot_bar(x1, y1, y2, x_label, 'stock', field, title, step)
