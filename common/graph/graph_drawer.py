...
def draw_bar(x, y, x_label, title):
    plt.figure(figsize=(20, 8), dpi=80)
...
# 绘制折线图和条形图
def draw_compare_plot_bar(x1, y1, y2, x_label, title):
    plt.figure(figsize=(20, 8), dpi=80)
    fig, axs = plt.subplots(2, 1)
    # 绘制数据
    axs[0].plot(range(len(x1)), y1, alpha=0.7)
...
def draw_default_compare_plot_bar(stock_df, title):
    x1 = stock_df['close'].index[::-1]
    y1 = stock_df['close'].values[::-1]
    y2 = stock_df['amount'].values[::-1]
    x_label = stock_df['trade_date'].values[::-1]

    draw_compare_plot_bar(x1, y1, y2, x_label, title)
